...
- Opening flights use a 340 ms easing curve. Outward flights use cubic ease-out over 340–1500 ms based on remaining distance, bounding initial speed to 2 progress units/sec. Completion uses animation callbacks. Opening another draft waits for its editor layout and saved scroll position to be ready. Input is held until the flight finishes; Reduce Motion switches directly.

Pinch distance sets a target rather than immediately moving the sheet. An interactive task advances toward it with a 160 ms smoothing time constant and a 1.8 progress units/sec speed cap, clamping elapsed time to 1/30 sec after stalls. Reversal updates the same target; release stops tracking and settles from the displayed progress. This prevents a large native event from skipping the visible flight. Physical trackpad feel remains to be checked in CAN-07.
