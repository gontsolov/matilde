...
| ID | Status | Item and completion criteria |
| --- | --- | --- |
| AI-01 | In progress | **Langdock foundation.** Implemented native HTTPS client, Keychain-backed settings, region/base URL and model discovery, bounded JSON-mode responses, cancellation and redacted errors. Manual review and local history are available. Transport is covered with synthetic tests; live authentication, model compatibility, latency, cost and quality remain unverified because no key was configured. See [Langdock integration](docs/langdock-integration.md). |
| AI-02 | In progress | **Review comments.** Writing assist now stays visible with an icon-free title menu and same-line Review/Cancel action. Review history is available from the title. Seven review tests, app/signature build and live light-mode header inspection passed. Manual Review now delivers up to three comments with optional accepted, undoable rewrites in floating cards using the standard UI body font. Compact and expanded quotes share the editor-style muted two-point left rule and 10-point indentation; build and live compact-card inspection passed. Exact quote navigation, dismissals and per-draft local history are implemented; branches start independent histories. Any content/title/goal edit invalidates prior suggestions and cancels pending reviews. Synthetic native acceptance/undo and persistence checks passed. Review cards now occupy a true overlay, reserving no layout column, positioned 16 points beyond the actual text edge. Stash overlays them; build/signature and live positioning/layering checks passed. Replaced the sidebar with floating passage-anchored cards, compact previews, expanded actions, selected-passage highlighting and collision stacking. Narrow windows use comment popovers. Live anchor alignment and expand/collapse checked; dark mode and narrow-window interaction remain verification gaps. Remaining: automatic idle reviews, pause, replies and finer anchor tracking. |
...
| EDIT-20 | Done | **Editable side-by-side comparison.** Open another draft beside the current one from draft/context menus; independent text buffers, cursor/scroll, undo, debounced autosave and external-edit recovery. Native resizable split; save on close/switch/quit. 80 Swift tests (one skip), app/signature build and live independent editing, per-pane undo, saving, divider resizing and close-flush checks passed with disposable drafts. Comparison selection is session-only; separate windows and synchronized scrolling remain future options. |
