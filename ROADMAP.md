...
| EDIT-14 | In progress | **Header keyboard flow and divider entry.** Implemented removal of editor header timestamp; Down/Enter/Tab move title → goal → body, Up from goal returns to title. Completing a standalone divider creates a following line with a caret. 60 Swift tests (one opt-in skip) and build/signature checks pass; live keyboard verification deferred when app detected owner interaction. |
